...
% [P, dPdx] = penalize(x, p, penal_scheme, xmin)
...
% consider input
    ...
    elseif n_inputs == 4
    	...
    	error('penalization received %d inputs, but requires 1 to 4',n_inputs)
    end
...
% define function
    switch penal_scheme
        ...
        case 'modified_SIMP'
            P = @(x) xmin + x.^p*(1-xmin);
            dPdx = @(x) (1-xmin)*p*x.^(p-1);
        case 'modified_RAMP'
            P = @(x) xmin + x./(1 + p.*(1-x))*(1-xmin);
            dPdx = @(x) (1-xmin)*((1+p)./(1 + p.*(1-x)).^2);            
        otherwise
